...
# Find desired group:
# requires group_id: use 3rd party site or parse FB group page
group_id = credentials[:group_id]
group_response = HTTP-get(
  "https://graph.facebook.com/v2.7/#{group_id}",
...
feed_response = HTTP.get(
  "https://graph.facebook.com/v2.7/#{group_id}/feed",
  params: { access_token: access_token })
...
attachment_response = HTTP.get(
  "https://graph.facebook.com/v2.7/#{posting_data['id']}/attachments",
  params: { access_token: access_token })
fb_response[:attachments] = attachment_response
attachment = JSON.load(attachment_response.to_s)
